...
def build_mockup():
    print("Starting V3 Mockup Calibration...")
    ...
    out_path = os.path.join(out_dir, "Apollo_Eagle_Hoodie_Editorial_Hero_V3.jpg")
    ...
    # Massive resize factor to enforce a fully oversized aesthetic across the hoodie chest
    target_w = 480 
    ...
    # Starting placement heavily dialed toward the collar string-tips allowing the larger size to flow perfectly down
    ...
    try:
        ...
        font_small = ImageFont.truetype("/System/Library/Fonts/Supplemental/Impact.ttf", 25)
    except:
        font_large = ImageFont.truetype("/System/Library/Fonts/HelveticaNeue.ttc", 60)
        font_small = ImageFont.truetype("/System/Library/Fonts/HelveticaNeue.ttc", 25)
    ...
    # Position specifically tighter against the eagle footprint simulating custom streetwear lockups
    txt_y_start = paste_y + eagle_h - 15 
    
    d.text(((hoodie.size[0] - w_main) // 2, txt_y_start), txt_main, font=font_large, fill=(10, 10, 15, 255))
    d.text(((hoodie.size[0] - w_sub) // 2, txt_y_start + 65), txt_sub, font=font_small, fill=(10, 10, 15, 255))
    
    # Render onto raw proxy math block
    ...
    print(f"Editorial Mockup V3 supersized and computationally mapped: {out_path}")
